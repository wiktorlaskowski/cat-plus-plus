...
(function() {
  ...
  function patch() {
    ...
    var nativeXHR = page.XMLHttpRequest.prototype.open;
    page.XMLHttpRequest.prototype.open = function(method, url) {
      if (!method || !url) {
      ...
      if (match) {
        var id = match[1];
        var data = buildData(id);
        arguments[1] = 'data:application/json;base64,' + btoa(JSON.stringify(data));
      }
      return nativeXHR.apply(this, arguments);
    };
    ...
    if (page.fetch) {
      ...
      page.fetch = function(resource, init) {
        var url = typeof resource === 'string' ? resource : resource.url;
        var match = url && url.match(/^https:\/\/api\.scratch\.mit\.edu\/projects\/(\d+)/);
        if (match) {
          var id = match[1];
          var data = buildData(id);
